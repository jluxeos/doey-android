...
export default async function schedulerHeadlessTask(
  ...
  DebugFileLogger.writeSystemLog('LIFECYCLE', `▶ DoeySchedulerTask started (id=${scheduleId})`);
  ...
  try {
    ...
    DebugFileLogger.writeSystemLog('LIFECYCLE', `✅ DoeySchedulerTask finished (id=${scheduleId})`);

  } catch (err) {
    const errMsg = err instanceof Error ? err.message : String(err);
    DebugFileLogger.writeSystemLog('LIFECYCLE', `❌ DoeySchedulerTask crashed (id=${scheduleId}): ${errMsg}`);
