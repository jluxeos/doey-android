 ...
 *
 * Brings DoeyBot's Activity to the foreground via an ACTION_MAIN Intent.
...
/**
 * Bring DoeyBot's Activity to the foreground.
 *
...
export async function bringToForeground(tag: string): Promise<void> {
  ...
  try {
    await IntentModule.sendIntent('android.intent.action.MAIN', null, 'com.doey', null);
  } catch (err) {
