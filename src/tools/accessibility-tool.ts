 ...
 * when DoeyBot goes to background). The result is delivered to DoeyBot later via appendPending.
 *
 ...
 *
 * Requires the user to enable "Doey" in Android Settings → Accessibility.
 */
...
export class AccessibilityTool implements Tool {
  ...
  description(): string {
    return (
      ...
      'The result will be delivered to DoeyBot later when the background task completes. ' +
      'This tool should typically be the LAST tool call in a tool loop, as any subsequent tool calls ' +
      'would execute before the accessibility automation finishes. ' +
  ...
  async execute(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (!serviceEnabled) {
      return errorResult(
        'The Doey Accessibility Service is not enabled. ' +
        'Please go to Android Settings → Accessibility → Doey and enable it. ' +
    ...
    // inside DoeyAccessibilityTask via HeadlessJsTaskService.  The JS runtime
    // there is NOT throttled when DoeyBot is in the background, unlike the
    // main UI JS context.
    const job = {
      packageName,
      goal,
